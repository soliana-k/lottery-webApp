...
const DrawManagement = () => {
  ...
  const { admin } = useSelector((store) => store.auth); // Access admin information from Redux store
  ...
  const handleCreateSubmit = async () => {
    try {
      const data = {
        ...
        time: drawTime,
        status: 'Upcoming',
        createdBy: admin.email, // Include admin email
      };
      const response = await axios.post('http://localhost:8000/api/v1/draws/create', data);
      setDraws([...draws, response.data]);
      handleCloseCreate();
      navigate('/draws'); // Redirect after successful creation
    } catch (error) {
      console.error('Error creating draw:', error.response ? error.response.data : error.message);
    }
  };
